...
class Ui_Form:
    ...
    def read(self):
        ...
        while self.vL_LinePlot_AI.count():
            item = self.vL_LinePlot_AI.takeAt(0)
            widget = item.widget()
        ...
        for _, item in self.file_list.items():
            df = Ppo.OilThick(item["path"]).dataframe
    ...
    def abrir_dialogo_arquivo(self, row, column):
        if column == 0:  # Apenas se clicar na primeira coluna
            caminho, _ = QFileDialog.getOpenFileName(self.main_frame_AI, "Selecionar Arquivo", "",
                                                      "Arquivos de Texto (*.txt *.log *.prt)")
            if caminho:
                ai_df = Ppo.OilThick(caminho)
        ...
        for col in self.line_variables:
            for _, item in self.data_files.items():
                if col.startswith("Polluting Mass"):
                    ...
                    for coldf in item.columns:
                        if not coldf.startswith("Polluting Mass") and coldf != self.x_value_col and col.split("[")[0] in coldf:
    ...
    def set_graph(self, **kwargs):
        ...
        if kwargs:
            ...
            mass_lists = [item[col] for _, item in self.data_files.items() for col in item.columns if col.startswith("Polluting Mass")]
            area_lists = []
            for _, item in self.data_files.items():
                for col in item.columns:
