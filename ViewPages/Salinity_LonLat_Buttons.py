...
class Ui_WindButton_LonLatProfile(object):
    ...
    def plot_profile(self):
        ...
        sali_filtered = dataset_filtered[self.sali_name].values

        if hasattr(self, 'im'):
            self.im.remove()

        self.im = self.ax.pcolormesh(self.lon2d, self.lat2d, sali_filtered, cmap=self.cmap, norm=self.norm,
    ...
    def first_profile(self):
        ...
        sali, sali_filtered = self.dataset[self.sali_name].values, dataset_filtered[self.sali_name].values
        ...
        # self.im = self.ax.imshow(sali_filtered, cmap=self.cmap, norm=self.norm, origin="lower",
        #                          extent=(min(lon), max(lon), min(lat), max(lat)), transform=ccrs.PlateCarree())
        # self.im.set_data(sali_filtered)
    ...
    def save_fig(self):
        ...
        ax.pcolormesh(lon2d, lat2d, salinity_filtered, cmap=cmap, norm=norm,
                      shading='auto', transform=ccrs.PlateCarree())
    ...
    def save_anim(self):
        ...
        self.img = self.axs.pcolormesh(self.lon2d_anim, self.lat2d_anim, data_first, cmap=self.cmap_anim,
                                       norm=self.norm_anim,
                                       shading='auto', transform=ccrs.PlateCarree())
