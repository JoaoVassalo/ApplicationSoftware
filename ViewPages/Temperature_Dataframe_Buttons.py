...
################################################################################
## Form generated from reading UI file 'Temperature_Dataframe_ButtonsSzuYAg.ui'
##
...
class Ui_WindButton_LonLatProfile(object):
    ...
    def average_temp_df(self):
        ...
        for t in range(len(self.dataset['time'].values)):
            ...
            df_temp[f'{t_formated}'] = [np.nanmean(self.dataset.water_temp[t, dpt, :, :]) for dpt in
                                        range(len(self.dataset.water_temp['depth'].values))]
