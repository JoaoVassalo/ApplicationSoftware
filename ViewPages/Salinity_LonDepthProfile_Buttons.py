...
################################################################################
## Form generated from reading UI file 'Salinity_LonDepthProfile_ButtonssIpSWF.ui'
##
...
class Ui_WindButton_LonLatProfile(object):
    ...
    def set_normvalues(self) -> tuple:
        vmax = np.nanmax(self.dataset.salinity[:, :, :, :])
        vmin = np.nanmin(self.dataset.salinity[:, :, :, :])
        return vmin, vmax
    ...
    def plot_var_profile_withdepth(self):
        ...
        dataset_to_plot = self.dataset.sel(lat=float(self.comboBox.currentText()), time=self.time_selected)

        var_value = dataset_to_plot.salinity.values
        depth = dataset_to_plot.depth.values
        x_axis = dataset_to_plot.lon.values
        ...
        ax.set_xlabel(f'Longitude [{self.dataset['lon'].attrs['units']}]', labelpad=5, fontsize=8, color='white')
    ...
    def save_fig(self):
        ...
        ax.set_xlabel(f'Longitude [{self.dataset['lon'].attrs['units']}]', labelpad=20, fontsize=18)
        ...
        plt.savefig(f'{path_to_save}\\DEPTHvsLON_SALINITY_{t_formated}_LATcoord{self.comboBox.currentText()}.png',
                    transparent=True)
        plt.close()
