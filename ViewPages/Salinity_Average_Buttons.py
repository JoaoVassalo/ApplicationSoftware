...
class Ui_WindButton_LonLatProfile(object):
    ...
    def plot_graph(self):
        ...
        ax.set_xlabel(f'Salinity [{dataset.units}]', labelpad=5, fontsize=8, color='black')

        ax.set_ylabel(f'Depth [{dataset[self.depth_name].units}]', labelpad=5, fontsize=8, color='black')
        ax.grid()
    ...
    def save_fig(self):
        ...
        plt.savefig(f'{path_to_save}\\AverageSalinity_{t_formated}.png', transparent=True)
        plt.close()
